...
角色数据存储在 `角色` 变量中，由 AI 在回复时通过 `<update>` 标签动态更新：

- AI 使用 JSON Patch `add` 操作添加新角色：`{ "op": "add", "path": "/角色/新角色名", "value": {...} }`
- UI 组件通过 `MvuBridge.getRoles()` 读取角色列表
- 组件订阅 `Mvu.events.VARIABLE_UPDATE_ENDED` 事件，在 AI 更新变量后自动刷新角色列表
...
例外：`CommonApps.tsx` 与 `HypnosisApp.tsx` 内有部分 MVU 事件订阅（`Mvu.events.VARIABLE_INITIALIZED` / `VARIABLE_UPDATE_ENDED`），属于“界面刷新触发器”，用于在 AI 回复更新 `角色` 变量后自动刷新角色列表。
