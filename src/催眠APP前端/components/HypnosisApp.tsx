...
export const HypnosisApp: React.FC<HypnosisAppProps> = ({ userData, onUpdateUser, onExit }) => {
  ...
  const refreshRoles = async () => {
    try {
      ...
      console.warn('[HypnoOS] Failed to refresh roles', err);
    }
  };
  ...
  // Subscribe to MVU variable updates for auto-refresh
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!ready) return;
        stops = [
          eventOn(Mvu.events.VARIABLE_INITIALIZED, () => refreshRolesRef.current()),
          eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, () => refreshRolesRef.current()),
        ];
      } catch {
